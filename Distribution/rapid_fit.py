...
def func(p):
  t = -s_values
  ret = np.log(p[0]**t + p[1]**t + p[2]**t + p[3]**t + p[4]**t)
  #ret = p[4]*loggamma(p[2] + p[3]*s_values)
  #ret += np.log(p[0]**2) 
  #ret += s_values*np.log(p[1]**2) #+ s_values**2 * np.log(p[4]**2) #+ s**3 * np.log(p[3]**2) + s**4 * np.log(p[4]**2)
