...
logmoments = np.load("logmoments_monte.npy")
s_values = np.load("s_values_monte.npy")
...
def spc(m,sr,si,*p):
  a = (m - np.imag(func(sr,si,*p)))**2
  b = (m - np.imag(func(sr,si,*p)) + 2*np.pi)**2
  c = (m - np.imag(func(sr,si,*p)) - 2*np.pi)**2
  return min(a,b,c)
...
def diff(p,real_s,imag_s,real_logm,imag_logm):
  ...
  ret = loss_real + loss_imag + reg1 + reg3
  print(ret)
  return ret
...
if(True):
  ...
  loss_imag = np.sum([ (m - np.imag(func(sr,si,*popt)))**2 for sr,si,m in zip(real_s,imag_s,imag_logm)]) 
  print(loss_real+loss_imag)
  
  ax = plt.axes(projection='3d')
  # Data for three-dimensional scattered points
  ax.scatter3D(real_s, imag_s, real_logm, c=real_logm, cmap='Reds', label = "Numeric")
  ax.scatter3D(real_s, imag_s, np.real(fit), c=np.real(fit), cmap='Greens', label = "Theoretical", s=16)
  ax.set_xlabel('Re(s)')
  ax.set_ylabel('Im(s)')
  ...
  ax.scatter3D(real_s, imag_s, np.imag(fit), c=np.imag(fit), cmap='Greens', label = "Theoretical", s=16)
  ax.set_xlabel('Re(s)')
  ax.set_ylabel('Im(s)')
  ...
  #plt.plot(s_values,logmoments,label="Data")
  #plt.plot(s_values,fit,label = "Fit")
  ##plt.plot(s_values,fit_2, label = "Ideal")
  #plt.legend()
  #plt.xlabel("s")
  #plt.ylabel("$\logE[x^{s-1}]$")
  #plt.show()
  p_best = popt
...
#def phi(k,p):
#  return (-1)**k * (p[0]**2)**(-k) * p[1]**2 * gamma(p[2] - p[3]*k)/gamma(k+1)/gamma(p[4]-p[5]*k)/gamma(p[6]-p[7]*k)

#p_best = [ 3.27130918e+00,  2.73772883e-02,  3.07138066e+01, -5.46383842e-01,3.56462660e+01,  1.12552981e+01,  7.86392962e+00,  5.82836630e+00,5.11316087e+01,  7.21763524e+00]

def f(x,p): 
  return np.sum(np.real([ (-1)**k * func_call(-k,*p_best) * x**k/gamma(k+1) for k in range(5)]))

f = [ f(q,p_best) for q in x]
