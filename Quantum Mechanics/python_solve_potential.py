...
V = np.array([ -200 * np.sqrt((xx+1)/2)*np.sqrt(1 - (xx+1)/2) if np.abs(xx)<=1 else 1e40 for xx in x])
...
s1 = np.random.uniform(low = 1, high =4, size = N_s)
...
np.save("s_values_Circ_200",s)
np.save("moments_Circ_200",q)
np.save("logmoments_Circ_200",np.log(q))
