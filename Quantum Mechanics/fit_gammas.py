...
logmoments = np.load("logmoments_Harmonic_pigu.npy")
moments = np.load("moments_Harmonic_pigu.npy")
s_values = np.load("s_values_Harmonic_pigu.npy")
...
def func(sr,si, *q):
  ...
  polynom = np.log(p[4] + p[5]*s + p[6]*s**2 + p[7]*s**3 + p[8]*s**4 + p[9]*s**5)
