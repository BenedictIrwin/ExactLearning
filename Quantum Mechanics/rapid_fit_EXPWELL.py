...
## Scaled
def func(p):
  ret = p[1]*loggamma(p[2] + p[3]*s_values)
  ret += np.log(p[0]**2) 
  ret += s_values*np.log(p[4]**2) # s_values**2 * np.log(p[6]**2) #+ s**3 * np.log(p[3]**2) + s**4 * np.log(p[4]**2)
  ret += np.log(1 + p[5]*s_values + p[6]*s_values**2 + p[7]*s_values**3 + p[8]*s_values**4)
