...
logmoments = np.load("logmoments_Cubic_0.npy")
moments = np.load("moments_Cubic_0.npy")
s_values = np.load("s_values_Cubic_0.npy")
...
## Scaled
def func(p):
  ret = loggamma(p[1] + p[2]*s_values)  -2*loggamma(p[3] + p[4]*s_values)
  ret += np.log(p[0]**2) 
  ret += s_values*np.log(p[5]**2) # s_values**2 * np.log(p[6]**2) #+ s**3 * np.log(p[3]**2) + s**4 * np.log(p[4]**2)
