...
class RMTclassifier:
  ...
  def Generate_Variational_Method(self, input_matrix, activity_column):
    ...
    for tau_value, sigma_value in self.tau_sigma_list:
      results = self.sigma_tau_matrix(sigma_value,tau_value,input_matrix,activity_column,"sigma_tau")
  ...
  ## Run the sigma tau routine for an input data matrix and activity column
  def Generate_Variational_Class_Method(self, input_matrix):
    
    num_rows = input_matrix.shape[0]
    ##num_tau = np.random.randint(4,num_rows)
    num_draws = 1000
    ##num_draws = int(50*np.ceil(num_rows/num_tau))
    print("Performing {} eigendecompositions!".format(num_draws))
    ...
    ## Store these parameters for easy access (i.e. the lists will become object properties)
    index = 1
    activity_column = np.array([0.0 for i in range(num_rows)])
    for _ in range(num_draws):
      results = self.sigma_tau_matrix(None,np.random.randint(4,num_rows),input_matrix,activity_column,"random")
      #print(results)
      self.st_eigenvalue_list.append(results[0])
      self.st_eigenvector_list.append(results[1])
      #self.st_mean_activity_list.append(results[2])
      ...
      self.st_drop_args_list.append(results[8])
      #self.st_activity_list.append(results[9])
      print("{}/{} with K = {}".format(index,num_draws,len(self.st_eigenvalue_list[-1])))
      index += 1
  ...
  def Variational_Prediction(self, input_row):
    ...
    #num_pairs = len(self.st_eigenvalue_list)
    ...
    return distances, self.st_activity_list
  ...
  def sigma_tau_matrix(self,sigma,tau,input_matrix,activity_column,mode): 
   
    if(mode=="random"):
      draw_size = tau
      index_list = [i for i in range(input_matrix.shape[0])]
      sample_indices = np.random.choice(index_list, size=draw_size, replace=False)
      data_slice = input_matrix[sample_indices,:]
      activity_slice = activity_column[sample_indices]
    if(mode=="sigma_tau"):  
...
for i in range(len(train_labels)):
  if(train_labels[i]==True): true_matrix.append(matrix[i])
...
print("Generating RMT True Model")
true_RMTmodel = RMTclassifier(true_matrix) ## Initiate a model using the training matrix
true_RMTmodel.Generate_Variational_Class_Method(true_matrix)

print("Generating RMT False Model")
false_RMTmodel = RMTclassifier(false_matrix) ## Initiate a model using the training matrix
false_RMTmodel.Generate_Variational_Class_Method(false_matrix)
...
randomRMTmodel.Generate_Variational_Class_Method(shuffled_matrix)
...
for power in [20]:
  ...
  pred_values = []
  for row in matrix:
    true_points   = true_RMTmodel.Variational_Prediction(row)
    false_points  = false_RMTmodel.Variational_Prediction(row)
    random_points = randomRMTmodel.Variational_Prediction(row)

    true_xs = true_points[0]
    false_xs = false_points[0]
    rand_xs = random_points[0]

    ## Get the smallest distance (embedding generated using the random shuffled columns)
    ## This is a test of the best noise can achieve
    ...
    for x in true_xs:
      if(x < min_rand_dist):
        weight = 1.0/(x**power)
        prediction_value += weight*1.0  ## 1.0 for true
        weight_sum += weight
    for x in false_xs:
      if(x < min_rand_dist):
        weight = 1.0/(x**power)
        prediction_value -= weight*1.0
        weight_sum += weight
    prediction_value /= weight_sum
    #prediction_value = round(prediction_value)
  
    true_values.append(train_labels[index])
  ...
  print(true_values)
  print(pred_values)
  for a,b in zip(np.array(true_values).astype(float),pred_values):
    print("{},{}".format(a,b))
