import numpy as np
from scipy.special import loggamma, gammaln, gamma, zetac
...
logmoments = np.load("logmoments_AiZeta.npy")
moments = np.load("moments_AiZeta.npy")
s_values = np.load("s_values_AiZeta.npy")
...
## Scaled
def func(p):
  ret = p[2]*loggamma(p[3] + p[4]*s_values)  + p[5]*loggamma(p[6] + p[7]*s_values) + np.log(zetac(p[8] + p[9]*s_values))
  ret += np.log(p[0]**2) 
  ret += s_values*np.log(p[1]**2) # s_values**2 * np.log(p[6]**2) #+ s**3 * np.log(p[3]**2) + s**4 * np.log(p[4]**2)
  #ret += np.log(1 + p[6]*s_values + p[7]*s_values**2)
