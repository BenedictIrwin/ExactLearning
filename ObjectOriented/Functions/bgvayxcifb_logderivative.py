...
def fp(p0,p1,p2,p3,p4,p5,p6):
  ...
  ret = np.add(ret,log(p1**2),out=ret,casting='unsafe')
  ret = np.add(ret,p3*digamma(p3*p2 + S*p3),out=ret,casting='unsafe')
  ret = np.add(ret,-p5*digamma(p5*p4 + S*p5),out=ret,casting='unsafe')
  ret = np.add(ret,-p6/(1.0 + S*p6),out=ret,casting='unsafe')
  return ret
