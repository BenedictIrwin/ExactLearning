...
def fp(p0,p1,p2,p3,p4,p5,p6):
  ...
  ret = 0
  ret = np.add(ret,0,out=ret,casting='unsafe')
  ret = np.add(ret,0,out=ret,casting='unsafe')
  ret = np.add(ret,p3**2*np.trigamma(p3*p2 + S*p3),out=ret,casting='unsafe')
  ret = np.add(ret,-p5**2*np.trigamma(p5*p4 + S*p5),out=ret,casting='unsafe')
  ret = np.add(ret,p6**2/(1.0 + S*p6)**2,out=ret,casting='unsafe')
  return ret
logderivative2 = frompyfunc(fp,7,1)
