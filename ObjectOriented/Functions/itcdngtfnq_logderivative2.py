...
def fp(p0,p1,p2,p3,p4,p5,p6,p7):
  ...
  ret = 0
  ret += 0
  ret += p2**2*np.trigamma(p2*p1 + S*p2)
  ret += p4**2*np.trigamma(p3 + S*p4)
  ret += -p6**2*np.trigamma(p5 + S*p6)
  ret += -np.trigamma(p7 + S)
  return ret
logderivative2 = frompyfunc(fp,8,1)
