...
import numpy as np, scipy.special as sps
...
def tetragamma(z,eps=np.spacing(np.double(1)),realmin=np.finfo(np.double).tiny):
    """ Tetragramma function of a complex argument for arg(z) < pi/2.
The trigramma and tetragamma functions are the only two polygamma functions
for which closed-form numerator coeffients are known [2].
...
"""
    ...
    zcomplex = isinstance(z,(complex,np.complex128,np.complex64))
    if (zcomplex and z.real < 0) or (not zcomplex and z < 0):
        return tetragamma(1-z) - 2*np.cos(np.pi*z)*(np.pi/np.sin(np.pi*z))**3
    if z == 0: return -np.inf
    # Use basic trigamma recurrence for small arguments
    if np.abs(z) < 10.5: return tetragamma(z+1) - 2/z**3
    zinv = 1/z
    return -zinv**2*(1 + zinv + 4*np.pi**2*tetragamma_g1(z,eps=eps,realmin=realmin))

def tetragamma_g1(z,maxits=40,eps=np.spacing(np.double(1)),realmin=np.finfo(np.double).tiny):
    z2 = z*z
    a1 = 1/(8*np.pi**2)
