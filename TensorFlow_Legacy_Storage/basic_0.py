...
## A function which represents the generalised f for moments expansion of hypergeometric functions
...
# Calculate dfdA by parts
def dlogfdA(lstar,mstar,psi_a_dot_matrix,psi_b_dot_matrix,kstar,A,D_Matrices):
  p_sum = sum( [ psi_a_dot_matrix[lstar,m] for m in range(num_p)] )
  q_sum = sum( [ psi_b_dot_matrix[lstar,m] for m in range(num_q)] )
  preterm = kstar[mstar] + sum([ A[lstar,q]*kstar[q]*D_Matrices[q][lstar,mstar] for q in range(num_dim)])
  return preterm*(p_sum-q_sum)

argc = len(sys.argv)

num_dim = 1  ## The number of dimensions of the predicited probability distribution (inputs + outputs)
num_exp = 100000
...
x0 = np.random.exponential(scale=1,size=1000)
...
S = np.random.exponential(scale=1.0,size=(num_exp,num_dim))
...
for s in S:
  ...
  dfdA = [[ dlogfdA(lstar,mstar,psi_a_dot_matrix,psi_b_dot_matrix,kstar,A,D_Matrices) for lstar in range(num_dim)] for mstar in range(num_dim)]
 
  print("dFdA = {}".format(dfdA))


  dLdA = 2.0*(logM - logE)*( dfdA + dPidA - Ainv.transpose())
