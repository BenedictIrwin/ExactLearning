...
def main():
  ...
  param_a_init  = tf.truncated_normal_initializer(mean=1.0, stddev=0.000000001)
  ...
  logXi_2s           = tf.map_fn( lambda s : tf.reduce_sum(tf.lgamma(s)), 2.0*S, dtype=tf.float32)
  ...
  logXi_a_minus_V_2s = tf.map_fn( lambda s : tf.reduce_sum(tf.lgamma(param_a + tf.tensordot(matrix_V,s,axes=[[1],[0]]))), 2.0*S, dtype=tf.float32) 
  logXi_b_minus_W_2s = tf.map_fn( lambda s : tf.reduce_sum(tf.lgamma(param_b + tf.tensordot(matrix_W,s,axes=[[1],[0]]))), 2.0*S, dtype=tf.float32) 
  ...
  norm_M = tf.exp(norm_logM) ## Get the raw moment for normalisation (dangerous?)
  ...
  norm_loss  = normalisation_weight * tf.losses.mean_squared_error(norm_M,norm_target)
  ...
  def get_exponent_samples():
    samps = np.ones([num_dim])+np.random.uniform(0.0,4.0,size=[num_train_exponent_samples,num_dim])
    drop_prob = 0.2
  ...
  with tf.Session() as sess:
    ...
    for epoch in range(EPOCHS):
      ...
      for _ in range(n_batches):
        ...
        loss_value, prnorm_M, ml, nl = sess.run([loss, norm_M, model_loss, norm_loss], feed_dict={ S:s_list, batch_size : num_rows, drop_prob : 0.5, training_bool : True})
        tot_loss += loss_value
    ...
    with open("Output_Mell_{}.txt".format(session_string), "w") as f:
      for i in range(500):
        ...
        E_obs, M_obs = sess.run([E, M], feed_dict={ S:s_list, drop_prob : 0.5, training_bool : True})
    ...
    with open("Output_Best_Models_Mell_{}.txt".format(session_string),"w") as f:
      ...
      f.write("E,")
      for i in range(num_best_models): f.write("M{},".format(i))
      f.write("\n")
      for i in range(500):
        s_list = [[1.0]]+np.random.exponential(scale=2.0,size=[1,num_dim])
        for sval in s_list[0]: f.write("{},".format(sval))
        E_obs = sess.run(E, feed_dict={ S:s_list, drop_prob : 0.5, training_bool : True})
        ...
        for model in range(num_best_models):
          ...
          sess.run(param_a_assign_op, feed_dict = {param_a_input : best_models_list[model][1][3]})
          sess.run(param_b_assign_op, feed_dict = {param_b_input : best_models_list[model][1][4]})
          M_obs = sess.run(M, feed_dict={ S:s_list, drop_prob : 0.5, training_bool : True})
