...
def main():
  ...
  matrix_M_init = tf.truncated_normal_initializer(mean=weight_initialiser_mean, stddev=weight_initialiser_std)
  param_a_init = tf.truncated_normal_initializer(mean=weight_initialiser_mean, stddev=weight_initialiser_std)
  param_b_init = tf.truncated_normal_initializer(mean=weight_initialiser_mean, stddev=weight_initialiser_std)
  param_c_init = tf.truncated_normal_initializer(mean=weight_initialiser_mean, stddev=weight_initialiser_std)
  ...
  #matrix_M = tf.get_variable(name="M", dtype=tf.float32, shape=[num_dim, num_dim], initializer=matrix_M_init)
  matrix_V = tf.get_variable(name="V", dtype=tf.float32, shape=[num_dim, num_dim], initializer=matrix_M_init)
  matrix_W = tf.get_variable(name="W", dtype=tf.float32, shape=[num_dim, num_dim], initializer=matrix_M_init)
  param_a  = tf.get_variable(name="a", dtype=tf.float32, shape=[num_dim], initializer=param_a_init) 
  param_b  = tf.get_variable(name="b", dtype=tf.float32, shape=[num_dim], initializer=param_b_init) 
  param_c  = tf.get_variable(name="c", dtype=tf.float32, shape=[num_dim], initializer=param_c_init) 
  ...
  matrix_M_inverse = tf.matrix_inverse(matrix_M)
  matrix_M_det = tf.matrix_determinant(matrix_M)
  abs_matrix_M_det = tf.abs(matrix_M_det)
  
 
  ## Print (possible issue)
  print("Possible issue where the constant terms do not have the right dimension?")
  logXi_a = tf.reduce_sum(tf.lgamma(param_a))  ## Hypergeometric normalisation coeffs
  logXi_b = tf.reduce_sum(tf.lgamma(param_b))
  logXi_c = tf.reduce_sum(tf.lgamma(param_c))
  
  ## Things that depend on the exponent sample
  k_star = tf.map_fn( lambda s: tf.tensordot(matrix_M_inverse,s,axes=[[1],[0]]), S, dtype=tf.float32)
  logXi_minus_k_star = tf.map_fn( lambda s: tf.reduce_sum(tf.lgamma(-k_star)), S, dtype=tf.float32)
  logXi_a_plus_k_star = tf.map_fn( lambda s : tf.reduce_sum(tf.lgamma(param_a + k_star)), S, dtype=tf.float32) 
  logXi_b_plus_k_star = tf.map_fn( lambda s : tf.reduce_sum(tf.lgamma(param_b + k_star)), S, dtype=tf.float32) 
  logXi_c_plus_k_star = tf.map_fn( lambda s : tf.reduce_sum(tf.lgamma(param_c + k_star)), S, dtype=tf.float32) 
  
  ## Calculate the log of the analytic moments (as a vector over samples in S)
  logM = logXi_c + logXi_a_plus_k_star + logXi_b_plus_k_star + logXi_minus_k_star - logXi_a - logXi_b - logXi_c_plus_k_star - abs_matrix_M_det
  ...
  k_star_norm = tf.tensordot(matrix_M_inverse,tf.ones([num_dim]),axes=[[1],[0]])
  logXi_minus_k_star_norm  =  tf.reduce_sum(tf.lgamma(-k_star_norm))
  logXi_a_plus_k_star_norm =  tf.reduce_sum(tf.lgamma(param_a + k_star_norm))
  logXi_b_plus_k_star_norm =  tf.reduce_sum(tf.lgamma(param_b + k_star_norm))
  logXi_c_plus_k_star_norm =  tf.reduce_sum(tf.lgamma(param_c + k_star_norm))
  
  norm_logM = logXi_c + logXi_a_plus_k_star_norm + logXi_b_plus_k_star_norm + logXi_minus_k_star_norm - logXi_a - logXi_b - logXi_c_plus_k_star_norm - abs_matrix_M_det
  ...
  #gamma_op    = optimiser.minimize(gamma_loss)
